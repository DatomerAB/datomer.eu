...
async function onRequestPost(context2) {
  ...
  const secretKey = env2.STRIPE_SECRET_KEY;
  if (!secretKey) {
  ...
  const origin = request.headers.get("origin") || env2.SITE_URL || "https://datomer.eu";
  try {
...
async function onRequestPost2(context2) {
  ...
  if (env2.WAITLIST_WEBHOOK_URL) {
    try {
      await fetch(env2.WAITLIST_WEBHOOK_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
// ../.wrangler/tmp/pages-bl9t99/functionsRoutes-0.5975189903691863.mjs
var routes = [
  {
